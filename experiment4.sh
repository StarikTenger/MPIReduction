...
echo "$(uniq $OAR_FILE_NODES | sed -n '3p') slots=1" >> hostfile
echo "$(uniq $OAR_FILE_NODES | sed -n '4p') slots=1" >> hostfile

for floats in 1000 1000000 200000000; do
    filename="experiment4_${floats}_floats.txt"

    echo "Experiment 4: reduction with ring, 4 nodes" > $filename
    echo "" >> $filename
